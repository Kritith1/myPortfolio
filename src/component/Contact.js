...
function Contact({title,secondtitle}) {
  ...
  const validate = (values) => {
    ...
    if(!values.email){
      errors.email= "email is recqired"
  ...
  return (
    ...
    <form class="form" onSubmit={handleSubmit}>
      ...
      <input type="text" class="name formEntry" placeholder="Name" name="fullName" onChange={handleChange} value={formValues.fullName} />
      <p>{formErrors.fullName}</p>

      <input type="text" class="email formEntry" placeholder="Email" name="email" onChange={handleChange} value={formValues.email} />
      <p>{formErrors.email}</p>

      <textarea class="message formEntry" placeholder="Message" name="message"></textarea>
      <p>{formErrors.textarea}</p>
      <button class="submit formEntry" type='submit' value="send">Submit</button>
    </form>
  </div></div>
  )
}
